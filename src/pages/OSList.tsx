  ...
  async function fetchOrders() {
    ...
    try {
      ...
      if (profile) {
        ...
        } else if (profile.role === 'TECH_ENG_CLINICA') {
          query = query.eq('target_sector_type', 'ENG_CLINICA');
        } else if (profile.role === 'TECH_PREDIAL') {
          query = query.eq('target_sector_type', 'MANUTENCAO');
        } else if (profile.role === 'SOLICITANTE') {
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            <span className="text-sm font-bold text-blue-600">105 total</span>
          </div>
        </div>
                ...
                <tr key={os.id} className="hover:bg-gray-50/50 transition-all cursor-pointer group">
                  <td className="px-6 py-5">
                    <div className="flex flex-col">
                      ...
                      <span className="text-[10px] font-bold text-gray-400 uppercase tracking-tighter mt-0.5">{os.target_sector_type.replace('_', ' ')}</span>
                    </div>
                  </td>
                  <td className="px-6 py-5">
                    <div className="flex flex-col max-w-xs">
                      <span className="text-sm font-bold text-gray-900 group-hover:text-blue-600 transition-colors truncate">{os.title}</span>
                  ...
                  <td className="px-6 py-5">
                    <div className="flex flex-col">
                      <span className="text-sm font-semibold text-gray-700">{os.location_detail}</span>
                  ...
                  <td className="px-6 py-5">
                    <div className="flex items-center gap-2">
                      <div className="w-7 h-7 bg-gray-100 rounded-full flex items-center justify-center text-[10px] font-bold text-gray-500 border border-gray-200 uppercase">
                        {os.requester?.full_name?.split(' ').map(n => n[0]).join('') || '?'}
                      </div>
                      <span className="text-xs font-semibold text-gray-600">{os.requester?.full_name || 'Desconhecido'}</span>
                    </div>
                  </td>
                  <td className="px-6 py-5 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <Link to={`/ordens-servico/${os.id}`} className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all opacity-0 group-hover:opacity-100">
      ...
      <div className="flex items-center justify-between px-2">
        <span className="text-sm text-gray-500 font-medium">Mostrando {orders.length} de {orders.length} ordens</span>
        <div className="flex items-center gap-2">
          <button className="px-4 py-2 text-sm font-bold text-gray-400 bg-white border border-gray-200 rounded-xl cursor-not-allowed">Anterior</button>
          <button className="px-4 py-2 text-sm font-bold text-blue-600 bg-white border border-blue-200 rounded-xl hover:bg-blue-50 transition-colors shadow-sm">Próxima</button>
        </div>
      </div>
    </div>
  );
