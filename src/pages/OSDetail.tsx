        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-sm font-medium text-gray-500 mt-0.5 italic">
              Aberta em {format(new Date(order.created_at), "dd/MM/yyyy 'às' HH:mm", { locale: ptBR })} por {order.requester?.full_name || 'Usuário'}
            </span>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm space-y-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 py-6 border-y border-gray-50">
              <InfoItem icon={MapPin} label="Localização" value={order.location_detail} />
              <InfoItem icon={Stethoscope} label="Setor" value={order.target_sector_type.replace('_', ' ')} />
              <InfoItem icon={Package} label="Equipamento" value={order.equipment?.name || 'N/A'} />
              <InfoItem icon={Tag} label="Patrimônio" value={order.asset_tag || order.equipment?.asset_tag || 'N/A'} />
            </div>
        ...
        <div className="space-y-8">
          ...
          <div className="bg-[#151619] p-6 rounded-2xl border border-white/5 shadow-xl text-white space-y-6">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between text-xs">
                ...
                <span className="font-bold">{format(new Date(order.created_at), "dd/MM/yyyy HH:mm", { locale: ptBR })}</span>
              </div>
              <div className="flex items-center justify-between text-xs">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col h-[500px]">
            <div className="p-5 border-b border-gray-50 flex items-center justify-between">
              ...
              <span className="px-2 py-0.5 bg-blue-50 text-blue-600 text-[10px] font-bold rounded-full">Recentes</span>
            </div>
            
            <div className="flex-1 overflow-y-auto p-5 space-y-6 scrollbar-hide">
              <Comment 
                user={order.requester?.full_name || 'Solicitante'} 
              ...
              {order.technician && (
                <Comment 
                  user={order.technician.full_name} 
                  time="--" 
            ...
            <div className="p-4 border-t border-gray-50">
              ...
              <div className="flex items-center gap-4 mt-3 px-1">
                ...
                <label className="flex items-center gap-2 cursor-pointer group">
                  ...
                  <span className="text-[10px] font-bold text-gray-400 group-hover:text-gray-600 transition-colors">Comentário Interno</span>
                </label>
              </div>
            </div>
          </div>
...
function InfoItem({ icon: Icon, label, value, isCritical }: any) {
  return (
    <div className="flex flex-col gap-1">
      ...
      <span className={cn(
        "text-sm font-bold",
        isCritical ? "text-rose-600" : "text-gray-800"
      )}>
        {value}
      </span>
    </div>
